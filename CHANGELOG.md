...
### Fixed
- #64: `scripts/build_zipapp.py` recorded a bare `git rev-parse HEAD` as `source_commit` with no dirty-tree check, so a build from an uncommitted tree silently claimed a commit its bundled code did not match. Both the embedded attestation and `PROVENANCE.json` now record `source_dirty` and a `working_tree_diff_hash` over the uncommitted state (tracked diff + untracked file contents), and `version --verify`/`doctor` surface a `source:` line (`DIRTY working tree`). Chosen policy is record-not-refuse: a refuse-on-dirty default would break the test suite's direct `build()` calls and routine local runs, since this repo is routinely dirty during development. Covered by a deliberate-dirty-tree build test plus controlled temp-repo unit tests for clean/dirty/untracked/unknown states.
