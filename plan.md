...
**Applied independently to both test tracks, honestly labeled.** This repository holds no real downstream Rust-target (Mode R) project and no real C++→Rust port (Mode P) project — confirmed: no `crates/` directory exists anywhere in this repository outside a test's own temporary workspace, and `schemas/examples/project-descriptor.*.example.json` are templates, not a real project's own descriptor. Producing a genuinely real selection for either track is therefore blocked on real project data this repository does not contain — reported here rather than papered over with a fixture standing in as if it were a real answer, the same "real machinery, exercised against a stand-in, the stand-in visible" precedent chainlink #24's never-compiled callsite fixtures and #26's Mode P gold-set fixture already set. With that limitation stated plainly, the evaluator was still run fresh, twice, independently, against two visibly-synthetic stand-in project inputs (`tests/test_select_pilot_cluster.py::TwoIndependentProjectsTest`) to demonstrate it produces a *different* answer for a *different* project's own data, never a shared or hardcoded name:
...
Both runs report zero input findings and select their own single eligible candidate; a real project's own descriptor and workspace, run through the identical `pipeline.py select-pilot-cluster`, is the remaining work before this section can honestly name a live pilot for either track.
