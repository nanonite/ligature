...
Scoped to the promotion's own `cluster` (external review, medium severity: an earlier version was workspace-wide, so a promotion for one cluster wrongly required witnesses belonging to unrelated clusters). A concept spec's own top-level `cluster` field is required by vendor/concept-to-code's schema and already read the identical way by `generate_feature_ledger.owning_cluster_for()` — reused directly here, not re-derived, so the two can never disagree about which cluster a feature belongs to.
