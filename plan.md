...
**Each feature's witness rendering is embedded, not linked.** When `witness_present` is true (the identical bar G18 already checked: genuinely valid witness, on-disk rendering, unambiguous), the feature's own generated SVG (`docs/witnesses/<concept>.<query>.svg`, chainlink #28) is read and embedded directly as a base64 `data:image/svg+xml` `<image>` — never a relative file reference, so the contact sheet stays one self-contained artifact that survives being opened from a different location or the underlying witness file moving. `witness_present: false`, or any read failure (belt-and-suspenders), renders an explicit dashed-border "rendering unavailable" placeholder instead of a blank space — the panel itself is never dropped: the declared feature set's own coverage must stay visible even when what is being reported is absence. A missing/failing regeneration (no `witness_backend` configured, or a real dispatch failure) leaves `determinism`/`degeneracy` honestly `not-checked`/`fail` while the EXISTING on-disk rendering, if any, still displays — a failed re-check must never hide what genuinely is on disk.
