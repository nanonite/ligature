...
The one required vendored file went through two more review rounds before landing. Round 2's fix walked every `scripts/*.py` file's AST for `Path(...) / "segment" / ...` join chains passing through a `"vendor"` segment resolving to a real on-disk file, replacing a hard-coded "exactly one, this pathname" assertion. Round 3's external review found that fix was still syntax-specific — it would miss `joinpath()`, `os.path.join()`, an f-string, an import alias, or the `importlib.resources` form #57 is expected to introduce, and it only counted a reference at all if the file it resolved to already existed, so a reference to a not-yet-created resource could evade detection on both sides at once. Resolved by not inferring requiredness from code shape at all: `scripts/vendored_resources.py` is now a small, explicit, hand-maintained registry (`VENDORED_RESOURCES: dict[str, str]`, one entry today) that `scripts/select_pilot_cluster.py` calls (`vendored_resource_path("concept_to_code_spec_schema")`) instead of building its own path. `VendoredResourceRegistryTest` checks the registry against the inventory bidirectionally BY CONTENT (no code scanning involved) and separately keeps a narrower AST scan as a bypass detector, asserting no *other* script constructs its own `vendor`-segment path outside the registry — verified directly, twice: injecting a second vendor reference and confirming the registry-vs-inventory check failed, then separately re-adding a raw `Path(...) / "vendor" / ...` chain to a scratch script and confirming the bypass detector caught it.
