...
**Implemented (chainlink #59)** as `docs/assumption-registry-schema.json` (v1.0) plus `scripts/assumption_registry.py`, in the four staged phases this section's design note called for. Phase A adds the registry (stable `assumption_id`, canonical `canonical_text`, a `content_hash` RECOMPUTED from that text rather than declared, `tracking_issue`, append-only `provenance`) under `specs/_assumptions/*.json` and validates it against every declared reference; composite references remain valid and authoritative (a workspace with no registry is completely unaffected). Phase B, via the boundary-contract and work-package-manifest `assumption_ref` `$def`s' new `oneOf`, dual-resolves `{"assumption_id": ...}` or the legacy `{boundary_id, tracking_issue, assumption_hash}` composite and emits a non-blocking `info` deprecation finding for the composite. Phase C is `ligature migrate --assumptions [--apply --reviewer <name>]`, a deterministic rewrite of generated/work-package references only — a reviewed boundary contract's own `assumptions[]` is reported as requiring the normal human approve checkpoint, never auto-rewritten. Phase D's removal is not attempted; the mechanism is `assumption_registry.ASSUMPTION_REF_LEGACY_REMOVAL_VERSION` (2.0) with `legacy_refs_supported()` refusing an unknown/unsupported registry major rather than reinterpreting it. The new G21 gate fails on a reference that is missing, ambiguous, hash-disagreeing, or dangling. `check_assumption_identity_collisions()` remains defense in depth, unchanged, and is not described as proving identity.
...
- **Implementation required, partially addressed (chainlink #6 audit) — the registry question itself stays open.** Composite assumption identity's real mechanical failure condition is not a cluster-count threshold at all (the issue's own original framing) but a workspace-wide **identity collision**: the same `tracking_issue` naming *different* `assumption_hash` values across boundary contracts — plan.md §8.4's own "one issue tracks several assumptions" trigger, made mechanically checkable. **Implemented** as `validate_boundary_contracts.check_assumption_identity_collisions()`, wired into `pipeline.py validate` (Stage 4) over every genuinely valid boundary contract discovered workspace-wide (`load_boundaries_by_id`, one call per crate, merged) — the cross-crate half of the tracking-issue-uniqueness check G1b already enforces within one boundary file. **Authority boundary, stated precisely so the claim isn't overstated:** a *disagreement* (same issue, different hashes) is fully mechanically decisive and reported as a hard error — two different declared strings under one issue number cannot both be right. An *agreement* (same issue, same hash) is deliberately left unflagged, but this does **not** mechanically establish shared assumption identity: `docs/boundary-contract-schema.json`'s `assumption_ref` has exactly three fields and stores no canonical assumption text anywhere; `assumption_hash` is a bare declared string, never recomputed by this pipeline from stored content, so a match proves only that the same string was typed at each site — as trustworthy as whoever typed it, not a cryptographic guarantee of identity. This check is therefore a real, narrow, useful mitigation for one observable failure mode, **not** the registry §8.4 says becomes mandatory once an assumption spans boundaries, survives a rename, or becomes a shared concept — those conditions, and whether they actually arise, remain to be observed from real project usage this repository does not yet have; §8.4's own registry guidance stands unchanged and unsuperseded by this audit.
- **General rule documented, project-agnostic (chainlink #6 audit); the one instance named in this plan remains externally blocked.** Whether a cluster with latent cross-verifier edges should be re-decomposed or carry a permanent degradation record is answerable in general, without naming any project's own cluster: **re-decompose** when the cross-verifier edges are a narrow boundary relative to the cluster's own intra-verifier edges (splitting along the verifier line does not fragment mostly-single-verifier work into disconnected pieces), each resulting half stays internally cohesive on its own (its own concepts remain connected via intra-cluster edges, not isolated singletons), and no single CG6 cycle spans both halves (a split that just relocates one cycle across a new crate boundary, still needing cross-verifier trust, has not reduced risk — decomposition must reduce risk, not relabel it); **keep a permanent degradation record** when the cross-verifier dependency is intrinsic to one conceptual unit (no split boundary exists that does not cut through an essential dependency) or when a hypothetical split would only recreate the identical cross-verifier boundary contract one level up. **Authority boundary, stated explicitly rather than left implied:** this is a **human-reviewed decision rule**, not a mechanically resolved one, and citing mechanical inputs below does not change that. "Narrow," "cohesive," and "intrinsic" are human judgments about a proposed split that no code here computes — nothing in this pipeline partitions a cluster's own concept graph, checks the resulting halves' internal connectivity, or measures how essential a cross-verifier dependency is to one conceptual unit. What mechanical inputs exist only *inform* that judgment, and require no new tooling: `gate_g14.check_closure_kind`'s own `_evidence_kinds` finding already names which verifiers are actually observed whenever `single_verifier_system` is `False`, and `select_pilot_cluster.py`'s per-cluster verifier/intra-edge extraction (chainlink #4) — pointed at any cluster, not only a pilot candidate — gives the concrete concept and edge counts a human weighs against the rule above. Computing the proposed split itself, its post-split connectivity, or a measure of intrinsic coupling is not implemented and is not claimed to be. plan.md §3's own `analysis-configuration` is a worked example from a hypothetical downstream project, not a real cluster this repository has evidence for (this repository holds no real Mode R or Mode P project at all — the identical blocker chainlink #50 already names for pilot selection); applying the rule to it, or to any other specific cluster, is chainlink #51's own scoped, linked-blocker work, not decided speculatively here.
...
- **New, not one of #6's original four, surfaced by a #6 review round:** this pipeline is a source checkout, not a distributable product. `scripts/pipeline.py` is invoked as `python3 scripts/pipeline.py ...` from inside this repository; there is no versioned `ligature` binary/package, no `ligature init --mode greenfield|port`, no owned/versioned skill bundle a target repository installs (the only `SKILL.md` anywhere in this tree is the vendored, read-only concept-to-code skill), no idempotent install/upgrade mechanism, no manifest distinguishing Ligature-owned files from user-owned ones, and no `doctor`/version-compatibility/migration story. This is exactly what a real Mode P pilot (chainlink #52) needs before it can bootstrap a genuinely independent workspace pinned to "a published snapshot of the pipeline," rather than a clone of ligature-workspace itself. Tracked as chainlink #53 ("Ship the Ligature metaharness binary with idempotent repository initialization and versioned LLM skills") — not implemented here, and not decided speculatively; scoped as its own critical issue rather than folded into this section's prose. #53 does not block #52 directly: the actual edge is #52 ← #60, the out-of-checkout acceptance gate that #53's own subissues (#56–#59) must pass before #60 runs, and #60 is what #52 is blocked by.
...
## 17. Metaharness distribution: inventory and CLI/state contracts

**Implemented** (chainlink #55, parent #53), in two rounds. This pipeline has so far been a growing collection of `scripts/pipeline.py` subcommands with no distribution story (#53's own gap analysis), no versioned project-state output, and a `status` command that means "what can this CLI do" rather than "what state is this project in." #55 is the reconciliation pass required before #56 (project-state engine), #57 (self-contained attested binary), #58 (init/skills/migration), and #59 (assumption registry) can be built against a fixed target instead of each inventing its own guess. It deliberately implements none of those four — it produces their shared inputs.

**Inventory.** `docs/implementation-inventory.json` (schema: `schemas/implementation-inventory.schema.json`, `inventory_version` "1.0") replaces the stale "53 tests / M0-M1" prose summary with the actual current counts, read directly off the repository rather than carried forward from memory: **1389 passing pytest cases** (173 subtests) across **45 test files**, **32 registered `pipeline.py` subcommands**, **40 runtime `scripts/*.py` modules**, milestones **M0-M4 all complete** (25/25 chainlink issues, per `chainlink milestone list` — plan.md's own M0-M4 references were always informal prose, never a tracked identifier, so this is the first place they're cross-checked against the actual issue tracker), and **19 implemented gates** (G1a/G1b across every validator, G2/G2+/G2++ per artifact type, G9, G11, G14-G20, R1, R2, CG3, CG6). Every entry carries a `disposition` (`runtime-required`, `installed-template`, `documentation`, `vendored-runtime`, `test-only`, ...) so #57's packager knows exactly what to bundle without re-deriving it — including the one genuinely non-obvious finding this pass produced: of all 42 files in the vendored `vendor/concept-to-code` submodule, exactly **one**, `schemas/spec.schema.json`, is required at runtime; the rest is a read-only audit pin, never a distribution asset.

`tests/test_inventory_drift.py` recomputes every enumerable set (registered CLI commands via a new `pipeline.build_parser()`/`pipeline.registered_commands()` split, `scripts/*.py`, `docs/*.json` + `schemas/*.schema.json`, `schemas/examples/*.json`, `prompts/*.md`, `tests/test_*.py`, `tests/fixtures/*/`) directly from the filesystem and argparse, and fails in both directions — an omission or a stale entry — rather than trusting a hand-maintained count. `tests/test_implementation_inventory_schema.py` covers the inventory schema's own validity/mutation-rejection behavior.

The one required vendored file went through three more review rounds before landing. Round 2's fix walked every `scripts/*.py` file's AST for `Path(...) / "segment" / ...` join chains passing through a `"vendor"` segment resolving to a real on-disk file, replacing a hard-coded "exactly one, this pathname" assertion. Round 3's external review found that fix was still syntax-specific — it would miss `joinpath()`, `os.path.join()`, an f-string, an import alias, or the `importlib.resources` form #57 is expected to introduce, and it only counted a reference at all if the file it resolved to already existed, so a reference to a not-yet-created resource could evade detection on both sides at once. Resolved by not inferring requiredness from code shape at all: `scripts/vendored_resources.py` is now a small, explicit, hand-maintained registry (`VENDORED_RESOURCES: dict[str, str]`, one entry today) that `scripts/select_pilot_cluster.py` calls (`vendored_resource_path("concept_to_code_spec_schema")`) instead of building its own path.

Round 4's external review found this registry fix still had two real gaps: nothing connected an actual `vendored_resource_path("key")` call site to a registered key (an unused registry entry, or a call naming an unregistered key, could both go unnoticed), and the bypass detector recognized only the pathlib `/`-chain syntax, missing `Path("vendor/foo")`, `joinpath("vendor", "foo")`, and any bare string literal. Both closed: `test_registry_keys_match_call_sites` scans every `vendored_resource_path(...)` call site across `scripts/*.py` and diffs the called-key set against `VENDORED_RESOURCES` bidirectionally; `_vendor_bypass_offenses_in` now also matches `.joinpath(...)`/`os.path.join(...)` call arguments, single-argument `Path(...)` calls, and a general non-docstring string-literal scan for `vendor/<segment>` patterns (docstrings are deliberately excluded — prose naming a file by path is not the same as constructing one; the one genuine non-docstring false-positive this surfaced, an f-string error message in `select_pilot_cluster.py` that repeated the schema's path as literal text, was fixed by interpolating the already-resolved path object instead, which is strictly more correct anyway since the message can no longer drift from the registry). `VendoredResourceRegistryTest` checks the registry against the inventory bidirectionally BY CONTENT (no code scanning involved for that half) and the AST-based bypass/call-site checks separately — verified directly, repeatedly: injecting an extra registered-but-uncalled key, injecting a call naming an unregistered key, and separately injecting each of the three new bypass forms (`Path("vendor/x")`, `joinpath("vendor", "x")`, `os.path.join("vendor", "x")`) into scratch scripts, confirming each one is caught before being removed.

Round 5's external review found one more gap in round 4's own broadened detector: `Path("vendor") / "pkg" / "file.json"` — a `Path(...)` call whose SOLE argument is the bare string `"vendor"`, then continued as the base of a `/`-chain — went undetected, since the `/`-chain scanner only ever inspects the right-hand segments it joins (never the call at the chain's own base) and the `Path(...)` call check only matched an argument *containing* `"vendor/"`, not one *equal to* `"vendor"` outright. Reproduced directly (`_vendor_bypass_offenses_in` returned `[]`) before fixing the `Path(...)` check to flag an argument that either equals `"vendor"` or contains `"vendor/"`. Round 5 also added `VendorBypassDetectorUnitTest`, nine tests exercising `_vendor_bypass_offenses_in` directly against synthetic scratch scripts (all five bypass forms, plus three legitimate non-bypass forms: a docstring mention, a real `vendored_resource_path()` call, and an unrelated `vendor`-free path) — pinning the detector's own behavior permanently rather than only incidentally through whatever real code happens to exist in `scripts/` today. Separately, `test_no_dynamic_vendored_resource_path_arguments` closes the reviewer's second observation: a `vendored_resource_path(...)` call whose argument isn't a literal string was previously skipped silently by the key-matching check; it now fails the build instead, on the same "a dynamic argument defeats the registry's whole premise" reasoning as any other bypass.

**CLI contract** (`docs/cli-contract.md`, grammar v1.0). Eleven top-level verbs: the ten #55's own issue text names (`init doctor version status check validate gate draft approve migrate`) plus one deliberate, justified addition, `report <report-id>`, holding four commands (`select-pilot-cluster`, `measure-gold-set`, `generate-feature-ledger`, `generate-contact-sheet`) that are none of validate/gate/draft/approve/check. `report` is **not** uniformly read-only (a first round of this contract wrongly said it was, then separately said two of its report-ids write, an internal contradiction an external review caught): `report pilot-cluster` writes nothing, but `report feature-ledger`/`report contact-sheet`/`report gold-set-measurement` (the last one by default, `--no-write` suppresses it) each write a generated projection file, documented per-report-id in §6's own table. All 32 of today's registered flat commands get exactly one disposition: 13 fold into `validate <artifact-kind>`, 6 into `gate <gate-id>`, 4 into `approve <operation>`, 4 into `report <report-id>`, 3 (`extract-c-static`, `check-bridges`, `render-witness`) become internal operations `check` may *recommend* via `next_action` but never runs itself, and the capability-summary `status` stub becomes `doctor`'s content — `status` is reserved exclusively for #56's real project-state query, unconditionally, from the product's first packaged release, not behind any version-gated transition window (a first round of this contract promised the OLD capability meaning "through product version 1.x" while §1's own grammar table already listed `status` as project-state at v1.0 — two meanings for one name under the same version; no packaged product has ever shipped either meaning publicly, so there was no real compatibility obligation to invent a transition window for). `tests/test_cli_contract.py` parses the contract doc's own mapping table and asserts every registered command has exactly one row and every stable alias target is used by exactly one legacy command; `tests/test_contract_consistency.py` asserts the specific resolved facts above directly (in both this doc and `docs/trust-and-compatibility-boundaries.md`) so none of the three round-1 contradictions can silently reappear.

The `check`-is-read-only fix matters beyond CLI wording: a first round of this contract said `check` "orchestrates [extract-c-static/check-bridges/render-witness] internally as needed," while `docs/trust-and-compatibility-boundaries.md` said `check` never writes and `consolidated-check.schema.json` required `mutated_workspace: const false` — three promises #56 cannot all implement at once, since all three of those commands genuinely write (`ci/results/c_static/`, `ci/results/bridge_checks/`, a witness's declared `output.path`). Resolved in favor of the schema and the trust document, not the CLI prose: `check` stays strictly read-only; when a gate's prerequisite data is missing or stale, `check` reports that gate `blocked` with a `reason` and may *recommend* the specific refresh command via `next_action`, but an operator (or a future orchestration layer outside `check` itself) decides whether to actually run it.

That fix's own first version had a follow-on defect a third review round caught: `next_action`'s only machine-consumable field, `command`, still pointed at those same three internal operations' flat names — the exact names §7 itself declares unversioned and "may change shape without notice." A consumer trying to act on `check --json` programmatically had nothing stable to key off after all. Resolved by splitting the concern `next_action` conflated: `action_id` (new, required whenever `kind` is `automated-command`, forbidden otherwise) is the stable, versioned identifier — `refresh-c-static`/`refresh-bridge-checks`/`refresh-witness` for the three internal operations, defined in `docs/cli-contract.md` §7's own table — while `command` becomes explicitly advisory-only, documented as never to be parsed or pattern-matched by automated tooling. `tests/test_contract_consistency.py::NextActionStableIdentifierConsistencyTest` pins this directly against both the schema and the doc.

Round 4's review found two more defects in that same fix. First: `action_id` was left an open pattern (any kebab-case string validated), which is not actually a versioned vocabulary — confirmed directly with `invented-unstable-action`, which passed. Closed to a JSON Schema `enum` of the four values this contract actually defines (the three `refresh-*` action_ids plus `author-interaction`, the action `schemas/examples/consolidated-check.blocked.example.json` already used); extending it is documented as an additive, minor schema-version change, same discipline as every other enumeration here. Second: the `command` field's own description claimed it "may become null" for an automated-command action, directly contradicting the conditional two lines below that always requires it non-null — the same self-contradiction, independently, in `docs/cli-contract.md`'s own prose ("command may be non-null, may be null"). Both false claims are removed; the actual requirement (always present, always non-null, for `automated-command`) was never in question and is unchanged. `tests/test_contract_consistency.py::ActionIdVocabularyConsistencyTest` pins the enum-matches-table and no-longer-self-contradicting facts directly.

**Project-state and consolidated-check contracts.** `schemas/project-state.schema.json` (the future `ligature status --json`) and `schemas/consolidated-check.schema.json` (`ligature check --json`), both v1.0, specify — not implement — #56's own output shape: per-artifact lifecycle (`absent draft invalid validated promoted stale-by-hash-drift`, plus an explicit `unknown` everywhere a check could fail to determine something), per-obligation required-vs-achieved assurance dimensions kept as separate arrays rather than collapsed into one boolean, per-cluster `closure_kind`/state/limitations, open findings/human-decisions/change-requests, gate-integrity, and — kept structurally separate from all of the above, per this plan's own witness-vs-assurance discipline (§16) — `generated_observations` for witness determinism/degeneracy summaries that must never stand in for an assurance verdict; the `assurance_dimension.dimension` field carries a schema-level `not: {const: "witness"}` after a two-step review chain: round 2 found an earlier draft merely LISTED `witness` as an example dimension in the description (contradicting the witness-vs-assurance separation in prose), and round 3, checking the actual fix, found the description-only correction still validated `{"dimension": "witness", "status": "achieved"}` with zero errors — the exact repro is now `test_witness_is_rejected_as_an_achieved_assurance_dimension`, and the broader vocabulary stays open (no enum) since only this one collision needed closing. The lifecycle/hash relationships are schema-enforced, not prose-only (a first round stated them only in a `description` field, which a review round correctly flagged as unenforced): `content_hash` is `const: null` exactly when `lifecycle` is `absent`, a real hashed string for every other declared lifecycle except `unknown` (which permits either, honestly modelling "we don't know"), and `promoted_hash` is required exactly when `lifecycle` is `promoted`/`stale-by-hash-drift` and forbidden (the key must be absent) otherwise — all four combinations, plus `unknown`'s two permitted content_hash states, covered directly by `ArtifactLifecycleInvariantTest` in `tests/test_project_state_schema.py`. `consolidated-check.schema.json` requires `mutated_workspace: const false` (schema-enforced, not just documented, read-only), models exactly-zero-or-one `next_action`, and ties its own `result.exit_code` to `docs/exit-code-contract.md`. A regression test (`tests/test_project_state_schema.py::OldCapabilitySummaryCannotSatisfySchemaTest`) proves the OLD flat `cmd_status` capability list cannot be reshaped into a document either schema accepts.

**Exit-code contract** (`docs/exit-code-contract.md`, v1.0). Reconciles the two conventions already live in the codebase — every standalone script's own `main()` returning 2 for a precondition error vs. `pipeline.py`'s dispatcher collapsing the same class of error to 1 via `PipelineError`, with `gate-r1-g16` alone using a real three-valued 0/1/3 today — into a five-code target (0 clean, 1 blocking findings, 2 invalid input/state, 3 human decision required, 4 backend unavailable) with a documented precedence order (2 > 1 > 4 > 3 > 0) implemented as a pure resolver, `scripts/exit_codes.py:resolve()`, exhaustively tested over the full condition powerset by `tests/test_exit_code_contract.py`. The doc records the current drift as known and explicitly unfixed by this task — wiring `pipeline.py`'s own dispatch to the reconciled codes is left to whichever of #56/#57/#58 next touches `main()`.

**Trust boundaries** (`docs/trust-and-compatibility-boundaries.md`). What's stable public API vs. explicitly not (module/function names), which commands are genuinely read-only (`report` removed from this list in round 2 — see above), which may write and under what authority (now including all three `report` write targets plus `extract-c-static`/`check-bridges`/`render-witness`'s own), and restates in contract form what the rest of this plan already establishes by construction: `status`/`check` cannot approve or promote, LLM output is advisory only, human checkpoints stay human, witnesses/differential agreement falsify but never prove, binary attestation is specified here and implemented under #57, and assumption-registry migration is specified under #59 — none of it invented early.

**Explicitly out of scope, left to later issues:** the real `assurance_dimension` vocabulary (open string in the schema; #56 closes it once the computation exists to enumerate against), whether `check`'s gate discovery is purely descriptor-driven (#56), packaged-resource discovery replacing every `Path(__file__)` lookup (#57), and the ownership-manifest three-way-comparison mechanics behind `installation_manifest` (#58) — `project-state.schema.json` only models the STATE such a manifest produces, not how #58 computes or stores it.
