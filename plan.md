...
**G19 is implemented** (chainlink #30) as `scripts/gate_g19.py`, wired as `pipeline.py gate-g19`. Unlike G18, it is not scoped to the declared `witness_required` set — every genuinely valid, promoted witness spec carries its own `determinism.value_hash` claim and every one of them is checked, the same way G9 checks every promoted bridge rather than only the ones a manifest references. There is no generic pluggable "witness producer backend" in the project descriptor the way `verifier_backends` exists for chainlink #47's bridge dispatch — a witness's value comes from evaluating a real query on a real fixture, which this repository cannot do generically. "Regenerate" is therefore the same split #47 already drew between `check_bridges()` (generates) and `gate_workspace()` (checks): something upstream of this gate re-runs the fixture producer and writes a fresh canonical result to `ci/results/witnesses/<witness_id>.json`; this gate is purely the comparison half, and a witness spec with no corresponding result blocks outright — `byte-identical-across-runs` is not established by a run that never happened. Nothing here re-hashes anything: `validate_witness.py`'s own `load_results_by_witness` already applies the "genuinely valid, not just present" bar to every canonical result, recomputing `value_hash` from the raw hashed fields via `witness_result.py`'s `compute_value_hash` and rejecting the file outright on disagreement, so a result's own `value_hash` field is already guaranteed self-consistent by the time this gate reads it — G19 only compares that field against the witness spec's normative `determinism.value_hash`. `render_hash` is never read anywhere in this module; a regression test proves a changed `render_hash` alone never blocks. A `witness_id` found valid under more than one crate is ambiguous and excluded, the identical choice #29 and #25 already make for their own cross-crate collisions.
