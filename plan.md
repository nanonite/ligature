...
**Every column is read from the gate that already owns it, computed fresh every run — never a cache of a stale one.** `witness_present` comes from `gate_g18.gate_workspace()`'s own per-feature disposition (genuinely valid witness, rendering exists on disk, unambiguous). `determinism` and `degeneracy` come from `gate_g19.gate_workspace()` (a REAL regeneration dispatch through the configured `witness_backend`, chainlink #30) and `gate_g20.gate_workspace()` respectively, matched to the feature's witness by `witness_id`; both gates already bundle their own cross-witness ambiguity findings into the same returned list their per-witness dispositions live in, so checking "any finding naming this witness_id" catches both without extra plumbing. `determinism`/`degeneracy` are `not-checked` (never `ok`/`pass` by default) when there is no witness to check at all, and `determinism` is specifically `not-checked` rather than `fail` when no `witness_backend` is configured — nothing was actually re-evaluated this run, a different fact from a real mismatch, distinguished by matching G19's own "no witness_backend configured" finding text. `implementation_observed` requires ALL three clean (`witness_present` and `determinism: pass` and `degeneracy: ok`) — the strict reading this issue's own requirements name, and the reason a stored result is never trusted merely because its `witness_id` matches (chainlink #30/#31's own hard-won lesson, inherited here by construction since it reuses the same gates rather than re-deriving their identity checks).
