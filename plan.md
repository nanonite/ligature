...
**Inventory.** `docs/implementation-inventory.json` (schema: `schemas/implementation-inventory.schema.json`, `inventory_version` "1.0") replaces the stale "53 tests / M0-M1" prose summary with the actual current counts, read directly off the repository rather than carried forward from memory: **1356 passing pytest cases** (159 subtests) across **45 test files**, **32 registered `pipeline.py` subcommands**, **39 runtime `scripts/*.py` modules**, milestones **M0-M4 all complete** (25/25 chainlink issues, per `chainlink milestone list` — plan.md's own M0-M4 references were always informal prose, never a tracked identifier, so this is the first place they're cross-checked against the actual issue tracker), and **19 implemented gates** (G1a/G1b across every validator, G2/G2+/G2++ per artifact type, G9, G11, G14-G20, R1, R2, CG3, CG6). Every entry carries a `disposition` (`runtime-required`, `installed-template`, `documentation`, `vendored-runtime`, `test-only`, ...) so #57's packager knows exactly what to bundle without re-deriving it — including the one genuinely non-obvious finding this pass produced: of all 42 files in the vendored `vendor/concept-to-code` submodule, exactly **one**, `schemas/spec.schema.json`, is read at runtime (by `scripts/select_pilot_cluster.py`, via a `Path(__file__)`-relative lookup #57's own issue text already names as a packaging blocker); the rest is a read-only audit pin, never a distribution asset.

`tests/test_inventory_drift.py` recomputes every enumerable set (registered CLI commands via a new `pipeline.build_parser()`/`pipeline.registered_commands()` split, `scripts/*.py`, `docs/*.json` + `schemas/*.schema.json`, `schemas/examples/*.json`, `prompts/*.md`, `tests/test_*.py`, `tests/fixtures/*/`) directly from the filesystem and argparse, and fails in both directions — an omission or a stale entry — rather than trusting a hand-maintained count. The one required vendored file is NOT hard-coded (an external review round found the first version was: it asserted "exactly one, this pathname" as a literal constant, which a new vendor/ reference could have evaded on both sides of the check); `VendoredAssetDriftTest` instead walks every `scripts/*.py` file's AST for `Path(...) / "segment" / ...` join chains passing through a `"vendor"` segment that resolve to a real on-disk file, and diffs that scanned set against the inventory's own `required_at_runtime` entries in both directions — verified directly by temporarily injecting a second vendor reference into a scratch script and confirming the test fails, then removing it. `tests/test_implementation_inventory_schema.py` covers the inventory schema's own validity/mutation-rejection behavior.
