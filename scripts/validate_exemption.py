...
def count_discovered(root: Path) -> int:
    ...
    the honest pass line (chainlink #48). Wraps find_exemption_files()
    rather than re-deriving its glob, so the count can never drift
    from the set actually validated."""
    return len(find_exemption_files(root))
...
def main(argv: list[str]) -> int:
    ...
    if not errors:
        print(pass_line(
            discovered, "exemptions", "G1a/G1b (incl. interaction cross-reference)", args.root
        ))
        return 0
